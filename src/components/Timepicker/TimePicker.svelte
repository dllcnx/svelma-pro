<script>
  ...
  $: {
    ...
    const sh = +time.toLocaleTimeString('en-US', { hour12: hour12, hour: 'numeric' }).split(' ')[0]
    ...
    {#if visible}
      <div class="touch-time-wrapper"  style="left: {left}">
        ...
        <div class="touch-time-picker">
          ...
          <Switcher type="seconds" data={SECONDS} selected={time.getSeconds()} on:timeChange={timeChanged} />
          {#if hour12}
            <Switcher
              type="meridiem"
              data={MERIDIEM}
